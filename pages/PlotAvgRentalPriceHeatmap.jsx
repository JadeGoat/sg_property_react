...
const PlotAvgRentalPriceHeatmap = ({ year, normalizeFlag }) => {
  ...
  useEffect(() => {
      if (data && data.length > 0) {
        ...
        const highestRentalPrice = data.reduce((highest, item) => {
          return item.monthly_rent_mean > highest.monthly_rent_mean ? item : highest;
        });
        ...
        for (const [town, [lat, lon]] of Object.entries(sgTowns)) {
          ...
          if (tempAvgData) {
            ...
            const heatmapValue = tempAvgData.monthly_rent_mean / highestRentalPrice.monthly_rent_mean
  ...
  return (
      <div>
          { normalizeFlag ? 
              <h2>Heatmap (Normalized Median)</h2>:
...
export default PlotAvgRentalPriceHeatmap